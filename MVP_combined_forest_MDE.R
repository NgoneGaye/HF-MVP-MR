...
# Combine — HFpEF on top, HFrEF below
...
# ── MDE reference lines (per outcome) ─────────────────────────────────────────
mde_data <- data.frame(
  outcome    = c("HFpEF", "HFpEF", "HFrEF", "HFrEF"),
  xintercept = c(1.28, 1/1.28, 1.22, 1/1.22)
) %>%
  mutate(outcome = factor(outcome, levels = c("HFpEF", "HFrEF")))
...
p <- ggplot(plot_data,
  ...
  # MDE bounds — per outcome panel
  geom_vline(data = mde_data,
  ...
  labs(
    ...
    caption  = paste0(
      ...
      "Estimates on log scale. Dashed line: null (OR = 1).\n",
      "Dotted red lines: minimum detectable effect at 80% power ",
      "(OR 1.28 / 0.78 for HFpEF; OR 1.22 / 0.82 for HFrEF)."
    )
  ) +
  
  # OR text labels to the right of each point
  geom_text(
    aes(x = or_uci95 * 1.05,
